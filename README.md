...
한국 특허청 **KIPRIS Plus REST API**를 활용하여 특허·실용신안 정보를 검색 및 엑셀(CSV)로 내보내고, **구글 Gemini AI**를 통해 특허 초록 요약 및 키워드 추출을 지원하는 웹 기반 분석 도구입니다. 부가적으로 특허 서지정보에서 IPC 상위 코드(IPC코드2)를 자동 매핑하여 보여줍니다.
...
2. **Google Gemini API Key (초록 요약 및 AI 키워드 추출용)**
   ...
   > **⚠️ 주의 (429/503 에러 및 RPM 한도 관련)**: 무료 요금제의 경우 짧은 시간에 과도한 요청 시 과부하(429/503) 차단이 발생할 수 있습니다. 
   > 이를 방지하기 위해 사용자 UI 화면에 **AI 요청 간격(Delay)** 조절 슬라이더(기본 4초)가 내장되어 있습니다. (Model: gemini-2.5-flash는 6.0초 이상 제안)
   >
   > 검색 완료 후 하단의 **"🤖 핵심요약·키워드 추출"** 버튼을 수동으로 클릭해야 AI 요약이 진행됩니다.
   >
